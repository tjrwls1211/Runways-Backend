    ...
    override fun createCourseByLLM(llmRequestDTO: LlmRequestDTO, userId: String): List<AutoGeneratedCourseDTO> {
        ...
        return try {
            ...
            if (response.statusCode.is2xxSuccessful && response.body != null) {
                ...
                val courses = responseBody["courses"] as? List<Map<String, Any>> ?: emptyList()

                courses.map { course ->
                    val lon = (course["position"] as List<Double>)[0]
                    val lat = (course["position"] as List<Double>)[1]
                    val positionNode = convertToGeoJsonPoint(lon, lat)
                    val coordinateNode = convertToGeoJsonLineString(course["coordinate"] as List<List<Double>>)

                    val location = locationApiService.getNearestLocation(lon, lat)
                    val sido = location?.sido ?: "Unknown"
                    val sigungu = location?.sigungu ?: "Unknown"

                    val tags = (course["tags"] as List<String>).map { tagName ->
