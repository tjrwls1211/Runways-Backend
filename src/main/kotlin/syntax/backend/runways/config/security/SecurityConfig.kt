    ...
    fun securityFilterChain(http: HttpSecurity): SecurityFilterChain {
        http
            ...
            .authorizeHttpRequests {
                it.requestMatchers("/","/oauth2/**", "/login/**","/api/user/validate","/ws/**","/app/**","/topic/**").permitAll()
                it.requestMatchers("/api/user/update", "/api/user/duplicatecheck").hasAnyRole("ADMIN", "USER", "WITHDRAWAL", "GUEST")
