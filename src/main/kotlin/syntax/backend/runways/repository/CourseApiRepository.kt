...
interface CourseApiRepository : JpaRepository<Course, UUID> {
    ...
    fun findByStatus(@Param("status") status: CourseStatus): List<Course>
    ...
    fun findByTitleContainingAndStatus(@Param("title") title: String, @Param("status") status: CourseStatus): List<Course>
}